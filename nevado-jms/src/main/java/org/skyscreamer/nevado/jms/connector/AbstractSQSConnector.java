...
/**
 * Abstract connector that handles handling of messages and queues independent
...
public abstract class AbstractSQSConnector implements SQSConnector {
  ...
  // TODO - Typica 1.7 doesn't support batch send. When we get a connector that
  // does, update this to allow implementation-specific bulk handler.
  public final void sendMessages(NevadoDestination destination, List<NevadoMessage> outgoingMessages)
      throws JMSException {
  ...
  public final NevadoMessage receiveMessage(NevadoConnection connection, NevadoDestination destination, long timeoutMs)
      throws JMSException {
  ...
  protected final SQSQueue getSQSQueue(NevadoDestination destination) throws JMSException {
    ...
    NevadoQueue queue = (destination instanceof NevadoQueue) ? (NevadoQueue) destination : ((NevadoTopic) destination)
        .getTopicEndpoint();
  ...
  protected final SQSMessage receiveSQSMessage(NevadoConnection connection, NevadoDestination destination,
      long timeoutMs, long startTimeMs, SQSQueue sqsQueue) throws JMSException {
    ...
    while (true) {
      if (connection.isRunning()) {
        ...
        if (!connection.isRunning()) {
          ...
          try {
            sqsQueue.setMessageVisibilityTimeout(sqsMessage.getReceiptHandle(), 0); // Make
                                                                                    // it
                                                                                    // immediately
                                                                                    // available
                                                                                    // to
            ...
            _log.warn(exMessage, e); // Non-fatal. Just means the message will
                                     // disappear until the visibility timeout
                                     // expires.
          }
          sqsMessage = null;
        }
  ...
  /**
   * Serialize a NevadoMessage object into the body of an SQS message into a
   * bae64 string.
   * 
   * @param message
   *          A NevadoMessage object
   ...
   *          String-serialized NevadoMessage
   * @return A deserialized NevadoMessage object
   * @throws JMSException
   *           Unable to deserializeFromString a single NevadoMessage object
   *           from the source
   */
   ...
   * 
   * @param snsArn
   *          ARN of SNS topic to subscribe to
   * @param sqsArn
   *          ARN of SQS queue subscriber
