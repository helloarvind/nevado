...
public class AsyncConsumerRunner implements Runnable {
  ...
  protected AsyncConsumerRunner(NevadoConnection connection) {
    ...
    _sleeper = new BackoffSleeper(1000, connection.getMaxPollWaitMs(), 1.5);
  }
  ...
  private boolean processMessage(NevadoMessageConsumer consumer) {
    ...
    if (consumer.getMessageListener() != null) {
      try {
        ...
        /*
         * ExceptionListener exceptionListener = null; try { exceptionListener = _connection.getExceptionListener(); }
         * catch (JMSException e1) { _log.error("Unable to retrieve exception listener from connection", e1); } if
         * (exceptionListener != null) { JMSException e = (t instanceof JMSException) ? (JMSException)t : new
         * JMSException(errorMessage + ": " + t.getMessage()); exceptionListener.onException(e); }
         */
      }
    }
